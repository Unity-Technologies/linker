...
namespace Mono.Linker
{
	public class TypeReferenceEqualityComparer : EqualityComparer<TypeReference>
	{
        ...
        public static bool AreEqual(TypeReference a, TypeReference b)
        {
            ...
            // If we resolved one, but not the other, then they can't be equal
            ...
            if (xDefinition != null && yDefinition != null)
	            // we can compare definitions by references because our AssemblyResolver ensures we only load each assembly once
                return xDefinition == yDefinition;

            // If we get here both of our resolved definitions are null.  Let's compare the best we can based on the TypeReferences
